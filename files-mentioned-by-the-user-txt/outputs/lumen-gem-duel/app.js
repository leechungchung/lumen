  ...
  app.innerHTML = `
    <main class="game-grid">
      <header class="top-bar">
        ...
        <div class="top-meta">
          <span class="room-pill">방 ${state.roomCode}</span>
          <span class="turn-pill">${current.name} 차례</span>
          <span class="score-pill">목표 명성 ${WIN_SCORE}</span>
        </div>
        <div class="top-actions">
          <button class="icon-btn" type="button" data-action="new-game" aria-label="새 게임">↻</button>
        </div>
      </header>
      ...
      <section class="main-layout">
        <div class="market-zone">
          <div class="zone-head">
            <h2>세공 시장</h2>
            <button class="mini-btn" type="button" data-action="collect" ${state.winnerId ? "disabled" : ""}>광맥 채굴</button>
          </div>
          ${marketHtml(current)}
        </div>
...
function selectedCardHtml(card, player) {
  ...
  return `
    <section class="selected-card gem-${card.color}" style="${gemStyle(card.color)}">
      ...
      <button class="primary-btn" type="button" data-action="buy" ${affordable && !state.winnerId ? "" : "disabled"}>
        세공 시작
      </button>
    </section>
  `;
...
function mineHtml(player) {
  ...
  return `
    <section class="mine-panel">
      ...
      <button class="ghost-btn" type="button" data-action="collect" ${state.winnerId ? "disabled" : ""}>광맥 채굴</button>
    </section>
  `;
}
          ...
          duel.awaitingStart
            ...
            : `
              ...
              <footer class="duel-controls">
                ${
                  duel.game === "sequence"
                    ? `<button class="tap-btn owner" type="button" data-duel-role="owner">세공 콤보</button><button class="tap-btn rival" type="button" data-duel-role="rival">견제 콤보</button>`
                    : `<button class="tap-btn owner" type="button" data-duel-role="owner">${duel.ownerButton}</button><button class="tap-btn rival" type="button" data-duel-role="rival">${duel.rivalButton}</button>`
                }
              </footer>
            `
        }
...
function duelBriefHtml(duel, owner, rival, rules) {
  return `
    <div class="duel-brief">
      ...
      <button class="primary-btn" type="button" data-action="start-duel">대결 시작</button>
    </div>
...
function resultHtml(result) {
  return `
    <section class="result-sheet">
      ...
      <button class="primary-btn" type="button" data-action="continue-result">보드로 돌아가기</button>
    </section>
  `;
}

function collectGems() {
  if (state.winnerId || state.phase !== "board") return;
...
function startDuelLoop() {
  ...
  duel.raf = requestAnimationFrame(duelFrame);
}
