...
async function createOnlineRoom() {
  ...
  game.roomCode = code;
  game.feed = [`방 ${code}가 생성되었습니다. 초대 코드를 공유해주세요.`];
...
// 개발 중에는 Service Worker가 예전 app.js를 붙잡아 화면이 멈출 수 있어서 등록하지 않습니다.
// 이미 등록된 Service Worker가 있으면 해제해 최신 파일을 바로 불러오게 합니다.
    ...
    <main class="game-grid">
      <header class="top-bar">
        ...
        <div class="top-actions">
          ${online.mode === "online" ? `<button class="mini-btn" type="button" data-action="copy-invite">초대 복사</button>` : ""}
          ${online.mode === "online" ? `<button class="mini-btn" type="button" data-action="new-room">새 방</button>` : ""}
          ${online.mode === "online" ? `<button class="mini-btn danger-lite" type="button" data-action="leave-lobby">로비로 나가기</button>` : ""}
...
function resetGame() {
  ...
  if (online.mode === "online") {
    const ok = confirm("현재 방 코드 그대로 새 판을 시작할까요? 상대 화면도 함께 초기화됩니다.");
    if (!ok) return;
  }
  state = createGame();
  if (online.mode === "online") {
    state.roomCode = online.roomCode;
    state.feed = [`방 ${online.roomCode}에서 새 판을 시작했습니다.`];
  }
  saveGame();
  render();
}
...
async function createFreshOnlineRoom() {
  if (!online.firebaseReady) return renderLobby("Firebase 설정이 아직 없습니다.");
  const ok = online.mode !== "online" || confirm("새 방을 만들까요? 기존 방과 다른 초대 코드가 생성됩니다.");
  if (!ok) return;
...
async function leaveToLobby() {
  const ok = online.mode !== "online" || confirm("현재 방에서 나가고 로비로 이동할까요? 로비에서 새 방을 만들거나 다른 방에 참가할 수 있습니다.");
  if (!ok) return;
  ...
  if (online.firebaseReady && online.db && code && idx >= 0) {
    try {
      await update(ref(online.db, roomPath(code)), {
  ...
  online.notice = "";
  state = null;
  renderLobby("로비로 나왔습니다. 새 방을 만들거나 다른 방 코드로 참가할 수 있습니다.");
}

function invitePanelHtml() {
  const code = escapeHtml(online.roomCode || state.roomCode || "");
  const hasGuest = Boolean(online.roomPlayers?.p2);
  const role = online.playerIndex === 0 ? "방장 · 세공사 A" : online.playerIndex === 1 ? "참가자 · 세공사 B" : "관전/로컬";
  const notice = online.notice ? `<div class="copy-notice">${escapeHtml(online.notice)}</div>` : "";
  return `
    <section class="invite-panel ${hasGuest ? "is-ready" : ""}">
      <div>
        <span class="tag">${hasGuest ? "ONLINE READY" : "INVITE"}</span>
        <h2>${hasGuest ? "상대가 참가했습니다" : "방이 생성되었습니다. 초대코드를 공유해주세요."}</h2>
        <p>${role} · 같은 링크로 접속한 뒤 아래 코드를 입력하면 같은 방에 들어옵니다.</p>
      </div>
      <div class="invite-code-box">
        <span class="invite-code">${code}</span>
        <button class="mini-btn" type="button" data-action="copy-code">코드 복사</button>
        <button class="primary-small" type="button" data-action="copy-invite">초대문 복사</button>
      </div>
      ${notice}
    </section>
  `;
...
async function copyText(text, successMessage) {
  try {
    ...
    prompt("복사가 막혔어요. 아래 내용을 직접 복사해주세요.", text);
    online.notice = "복사 권한이 막혀 직접 복사 창을 열었습니다.";
  }
  render();
}
...
function copyInvite() {
  ...
  copyText(inviteText(), "초대 링크와 방 코드를 복사했습니다.");
}

function copyRoomCode() {
  if (!online.roomCode) return;
  copyText(online.roomCode, "방 코드만 복사했습니다.");
}
